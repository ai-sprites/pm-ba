...
资料 host repo 和产物 repo 已明确的直接沿用，缺项合并用白话问一次，允许稍后配置。本角色业务产物统一保存在业务仓库的 docs/pm-ba/，功能目录放在角色目录内。需要跨 Git 资料桥接时按手册自动补齐并告知我，不另外索取安装确认。
...
> 请用 sprite-pm-ba，把这份功能简报整理到能交给团队评审的程度，保存到项目 docs/pm-ba/。待决定的问题直接问我。
...
需求正文、用户故事、验收和必要附件统一保存在指定业务仓库的 `docs/pm-ba/`；PRD 使用 `docs/pm-ba/prd.md`，独立故事使用 `docs/pm-ba/stories/<ID>-<name>.md`，多个功能可在角色目录内分目录。本次更新的旧散落产物迁入该角色目录并同步链接、已有索引和消费引用，保留用户内容，目标冲突先比对合并。核对真实路径、依据和待确定项；角色安装目录与业务产物目录分开。
