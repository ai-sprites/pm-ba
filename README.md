...
```text
请阅读 https://github.com/ai-sprites/pm-ba 的 README、详细接入手册和资源清单，将 sprite-pm-ba 接入当前项目。
...
需要跨会话或跨项目复用时，直接让 AI 把需求正文、用户故事与验收保存到你指定的业务仓库/目录，或沿用项目已有位置，核对真实路径、依据和待确定项。已有文件优先；没有位置约定时可用 `docs/artifacts/<feature-id>/requirements.md`。角色安装目录与业务产物目录分开。
